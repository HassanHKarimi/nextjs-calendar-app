...
# Create a Next.js config file that Vercel will actually use during build
cat > next.config.js << 'EOF'
/** @type {import('next').NextConfig} */
...
# Make sure the config file is in the root directory and is properly detected by Next.js
echo "Next.js config contents are:"
cat next.config.js
...
if [ "$VERCEL" = "1" ]; then
  ...
  # If routes-manifest.json wasn't created in dist, let's look for where it might be
  if [ ! -f "./dist/routes-manifest.json" ]; then
    echo "Routes manifest not in dist, trying to find it elsewhere:"
    find ./ -name "routes-manifest.json" -type f
    
    # Create the routes-manifest.json in the dist directory if needed
    if [ -f "./.next/routes-manifest.json" ]; then
      echo "Found routes-manifest.json in .next, copying to dist"
